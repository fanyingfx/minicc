  ...
  | For { init; condition; post; body; _ } ->
      let typecheck_for_init = function
        | InitDecl {storage_class =Some _;_}->
          failwith "Storage class not permitted on declaration in for loop header"
...
(* Symbols.add_var name ~t:Int; opt_typecheck typecheck_exp init *)

and typecheck_func_decl { name; params; body; storage_class } =
    ...
    if t <> Int then failwith "Function redeclared as variable"
      ...
      | StaticAttr { global = prev_global; init = prev_init } ->
          let global =
            ...
            else failwith "Conlicting variable linkage"
          in
          let init =
...
let typecheck (Ast.Program fn_decls) = List.iter typecheck_global_decl fn_decls
