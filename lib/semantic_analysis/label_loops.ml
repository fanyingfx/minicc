...
let rec label_statement current_label = function
  ...
  | Continue _ -> (
      match current_label with
      | Some l -> Continue l
      | None -> failwith "Continue outsie of loop")
  | While while_loop ->
  ...
  | DoWhile dowhile_loop ->
      let new_id = Unique_ids.make_label "dowhile" in
      DoWhile
        {
          dowhile_loop with
          body = label_statement (Some new_id) dowhile_loop.body;
          id = new_id;
        }
  | For for_loop ->
...
let label_loops (Program fn_defs) = Program (List.map label_function_def fn_defs)
