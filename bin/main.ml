...
let run_command cmd args =
  if Sys.command (Filename.quote_command cmd args) <> 0
  then failwith ("Command failed: " ^ cmd)
;;
...
let driver debug stage src =
  ...
  if stage = Settings.Executable
  then assemble_and_link ~cleanup:(not debug) assembly_name
  else ()
;;
...
let debug =
  let doc =
    "Write out pre- and post-register-allocation assembly and DOT files of interference \
     graphs."
  in
  Arg.(value & flag & info [ "d" ] ~doc)
;;

let src_file = Arg.(required & pos 0 (some non_dir_file) None & info [] ~docv:"files")
